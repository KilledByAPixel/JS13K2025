...
function debugRender()
{
    ...
    if (debugTakeScreenshot)
    {
        ...
        mainContext.drawImage(mainCanvas, 0, 0);
...

    {
        ...
        debugPrimitives.forEach(p=>
        {
            ...
            mainContext.scale(1, p.text ? 1 : -1);
            mainContext.fillStyle = mainContext.strokeStyle = p.color;

            if (p.text != undefined)
            {
                mainContext.font = p.size*cameraScale + 'px '+ p.font;
                mainContext.textAlign = 'center';
                mainContext.textBaseline = 'middle';
                mainContext.fillText(p.text, 0, 0);
            }
            ...
            else if (p.size.x != undefined)
            {
                ...
                const w = s.x, h = s.y;
                p.fill && mainContext.fillRect(-w/2|0, -h/2|0, w, h);
            ...
            else
            {
                // circle
                mainContext.beginPath();
                mainContext.arc(0, 0, p.size*cameraScale, 0, 9);
                p.fill && mainContext.fill();
                mainContext.stroke();
            }
    ...
    if (debugOverlay || debugPhysics || debugParticles || debugRaycast || debugGamepads)
    {
        // draw debug overlay
        mainContext.save();
        ...
        if (debugOverlay)
        {
            ...
            mainContext.fillText('ESC: Debug Overlay', x, y += h);
            ...
            mainContext.fillText('3: Debug Gamepads', x, y += h);
            mainContext.fillStyle = debugRaycast ? '#f00' : '#fff';
            mainContext.fillText('4: Debug Raycasts', x, y += h);
            mainContext.fillStyle = '#fff';
            ...
            keysPressed && mainContext.fillText('Keys Down: ' + keysPressed, x, y += h);
            ...
            buttonsPressed && mainContext.fillText('Gamepad: ' + buttonsPressed, x, y += h);
        }
        else
        {
            mainContext.fillText(debugPhysics ? 'Debug Physics' : '', x, y += h);
            mainContext.fillText(debugParticles ? 'Debug Particles' : '', x, y += h);
            mainContext.fillText(debugRaycast ? 'Debug Raycasts' : '', x, y += h);
