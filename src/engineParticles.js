...
class ParticleEmitter extends EngineObject
{
    ...
    emitParticle()
    {
        ...
        const pos = this.pos.add(
            typeof this.emitSize === 'number' ? // check if number was used
        ...
        const velocityAngle = this.angle + coneAngle;
        
        // build particle
        const particle = new Particle(pos, this.tileInfo, angle, colorStart, colorEnd, particleTime, sizeStart, sizeEnd, this.fadeRate, this.additive, this.trailScale);
...
class Particle extends EngineObject
{
     ...
     */
    constructor(position, tileInfo, angle, colorStart, colorEnd, lifeTime, sizeStart, sizeEnd, fadeRate, additive, trailScale
    )
    { 
    ...
    render()
    {
        ...
        const pos = this.pos, angle = this.angle;
        if (this.trailScale)
        {
            ...
            if (speed)
            {
                ...
                drawTile(pos.add(direction.multiply(vec2(0,-trailLength/2))), size, this.tileInfo, color, direction.angle(), this.mirror);
            }
        }
        else
